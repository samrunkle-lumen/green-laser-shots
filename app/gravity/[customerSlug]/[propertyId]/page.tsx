...
export default async function GravityPropertyPage({ params }: PageProps) {
  const { customerSlug, propertyId } = await params;

  // Gravity property pages are always for the gravity partner
  ...
  const customerUrl = `/gravity/${customer.slug}`;
  ...
  const propertyUrl = `${baseUrl}/gravity/${customerSlug}/${propertyId}`;
