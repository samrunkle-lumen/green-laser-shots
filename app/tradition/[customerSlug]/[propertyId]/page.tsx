...
export default async function TraditionPropertyPage({ params }: PageProps) {
  const { customerSlug, propertyId } = await params;

  // Tradition property pages are always for the tradition partner
  ...
  const customerUrl = `/tradition/${customer.slug}`;
  ...
  const propertyUrl = `${baseUrl}/tradition/${customerSlug}/${propertyId}`;
