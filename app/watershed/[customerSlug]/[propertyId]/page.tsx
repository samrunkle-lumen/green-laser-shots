...
import { getPropertyById, groupPropertiesByCustomer, getCustomerBySlug } from '@/lib/utils/aggregations';
...
export default async function WatershedPropertyPage({ params }: PageProps) {
  const { customerSlug, propertyId } = await params;

  // Watershed property pages are always for the watershed partner
  ...
  // Get customer data for breadcrumb
  ...
  const customerUrl = `/watershed/${customer.slug}`;
  ...
  const propertyUrl = `${baseUrl}/watershed/${customerSlug}/${propertyId}`;
